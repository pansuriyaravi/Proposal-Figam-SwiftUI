...
struct InternalWrappingHStack: View {
        ...
        self.firstItemOfEachLine = {
            ...
            for (index, element) in contentManager.items.enumerated() {
                ...
                case .any where contentManager.isVisible(viewIndex: index):
                    ...
                    if isScollable {
                        // Ignore last element's width
                        // Since small part of it will be visible
                        // That can be scrolled
                        if currentWidth + spacing.minSpacing + horizontalSpacing >= width {
    ...
    var totalLines: Int {
        return firstItemOfEachLine.count
    }
                ...
                if isScollable {
                    ScrollView(.horizontal, showsIndicators: false) {
                        WarpperHStack(
                            ...
                            firstItemOfEachLine: firstItemOfEachLine,
    ...
    private struct WarpperHStack: View{
        let lineIndex: Int
        let width: CGFloat
        let alignment: HorizontalAlignment
        let spacing: WrappingHStack.Spacing
        ...
        func startOf(line i: Int) -> Int {
            return firstItemOfEachLine[i]
        }

        func endOf(line i: Int) -> Int {
            return i == totalLines - 1 ? contentManager.items.count - 1 : firstItemOfEachLine[i + 1] - 1
        }
        
        func hasExactlyOneElement(line i: Int) -> Bool {
            return startOf(line: i) == endOf(line: i)
        }
    }
}
